...
test_that("parse_info", {
  ...
  dat <- con$INFO()
  expect_that(parse_info(dat), is_a("list"))
  expect_that(redis_version(con), equals(info$redis_version))
})
...
test_that("redis_time", {
  ...
  expect_that(redis_time(con), is_a("character"))
  expect_that(redis_time_to_r(redis_time(con)), is_a("POSIXt"))
})
