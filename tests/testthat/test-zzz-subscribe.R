...
test_that("higher level", {
  ...
  con <- hiredis()
  vals <- con$subscribe(ch, transform, terminate)
  expect_that(length(vals), is_more_than(0))
...
test_that("flood and recover", {
  ...
  con <- redux::redis_connection(redis_config())
  ptr <- environment(con$command)$ptr

  ## Directly from the lower-level things
  res1 <- try(.Call(redux:::Credux_redis_subscribe, ptr, ch, fn, .GlobalEnv),
              silent=TRUE)
  res2 <- .Call(redux:::Credux_redis_unsubscribe, ptr, ch)
  ...
  expect_that(res <- con$subscribe(ch, fn, .GlobalEnv),
